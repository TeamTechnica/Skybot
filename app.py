...
def get_cost(location, num_passengers):
    ...
    if location.upper() == "JFK":
        ...
        jfk_cost = '%.2f' % ((json.loads(jfk.text)['cost_estimates'][1]['estimated_cost_cents_max']/100) / num_passengers)
        return '$'+str(jfk_cost)
    elif location.upper() == "LGA":
        ...
        lga_cost = '%.2f' % ((json.loads(lga.text)['cost_estimates'][1]['estimated_cost_cents_max']/100) / num_passengers)
        return '$'+str(lga_cost)
    elif location.upper() == "EWR":
        ewr = requests.get('https://api.lyft.com/v1/cost?start_lat=40.8075&start_lng=-73.9626&end_lat=40.6895&end_lng=-74.1745')
        ewr_cost = '%.2f' % ((json.loads(ewr.text)['cost_estimates'][1]['estimated_cost_cents_max']/100) / num_passengers)
        return '$'+str(ewr_cost)
    else:
        return "Cost could not be calculated"
