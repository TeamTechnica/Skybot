...
def matchFound(cur_user, cur_fltDate, cur_fltTime, cur_airport, cur_maxPass):
    ...
    if current_maxPass == 1:
        matched_flight = (Flight.query.filter(
            Flight.flight_date == current_fltDate, Flight.departure_time.between((current_fltTime - 100),(current_fltTime + 100)), Flight.airport == current_airport, (Flight.match_id == None)).first()) # getting all flights with the same departure date
    ...
    if current_maxPass == 2:
        ...
        matched_flight = (Flight.query.filter(
            Flight.flight_date == current_fltDate, Flight.departure_time.between((current_fltTime-100),(current_fltTime + 100)), Flight.airport == current_airport, (Flight.match_id == None)).first())
        ...
        if matched_flight == None:
            ...
            previously_matched_flights = Flight.query.filter(Flight.flight_date == current_fltDate, Flight.departure_time.between((current_fltTime),(current_fltTime + 100)), Flight.airport == current_airport, (Flight.match_id != None))

            for x in previously_matched_flights:     
                
                match_instance = Match.query.filter(Match.id == x.match_id).first()
    ...
    # Otherwise
        ...
        if matched_flight.match_id == None:
            ...
            matched_user = User.query.filter(User.id == matched_passenger_id).first()
            match_availableSeats = (min(int(current_maxPass), int(matched_user.max_passengers))) - 1

            # Finds the rider with the earliest departure time and subtracts two hours
            match_departTime = (min(int(current_fltTime), int(matched_flight.departure_time))) - 200
        ...
        # If user was matched to a previously matched flight    
            ...
            shared_match = (Match.query.filter(Match.id == shared_match_id)).first()
            ...
            earliest_flight = (Flight.query.filter(Match.id == shared_match_id).order_by(Flight.departure_time)).first()
