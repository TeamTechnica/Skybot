...
#print ("This is the list of Flights who have the same testDate and testAirport")
...
test_date2 = 12252018
test_airport2 = 'LGA'
matches = session.query(Flight).filter(Flight.flight_date == test_date2, Flight.airport == test_airport2)
if (session.query(matches.exists()).scalar()):
	pass
	#print ("The following flights match the second testDate and testAirport: ")
	for hit in matches:
		pass
		#print(hit)
...
#connect the users (uni) who's flight's match requirements (max passengers)


#testing the backref relationship between User and Flights
new_test_flight1 = Flight(id = 7, airport = 'JFK', flight_date = 10312018, departure_time = 122800, user_id = 2)
new_test_flight2 = Flight(id = 8, airport = 'JFK', flight_date = 10312018, departure_time = 122800, user_id = 4)
